...
import matcher.score.logic as score
...
def process_file(repo_file_path):

    file_path = get_csv_file_from_storage(repo_file_path)
    ...
    if data.file_already_proccessed(file_hash):
        ...
        records_with_candidates = data.infer_candidates(input_records)
...
def process_rows(file_hash, records_with_candidates):
    for record in records_with_candidates:
        ...
        for candidate in record['candidates']:
            match_score = score.calculate_match_score(record, candidate)
            # log.info('the candidate => {} with SCORE: {}'.format(
            #     candidate, match_score
            # ))
            if match_score > MIN_ACCEPTABLE_SCORE:
                data.create_record_match(
                    file_hash,
                    record,
                    candidate,
                    match_score
